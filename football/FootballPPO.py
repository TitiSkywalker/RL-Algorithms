...
class FootballAgent():
    ...
    def action_value(self, parameter, minimap, action = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for episode in range(num_train_episodes):
            ...
            while True:
                ...
                with torch.no_grad():
                    ...
                    action, prob_logit, entropy, value = self.action_value(current_parameter, current_minimap)
                    values.append(value.flatten())
                    actions.append(action)
                    prob_logits.append(prob_logit)
                # batched environmnts
                next_parameter, next_minimap, reward, terminated, info = envs.step(action.cpu().tolist())
            ...
            for optimize_epoch in range(self.num_optimize_epochs):
                ...
                for start_index in range(0, batch_size, self.minibatch_size):
                    ...
                    # compute policy loss
                    _, new_prob_logits, new_entropy, new_value = self.action_value(mb_parameters, mb_minimaps, mb_actions)
            ...
            print(f"Training episode {episode}, mean reward = {torch.mean(current_rewards).item()}, max reward = {torch.max(current_rewards).item()}, min reward = {torch.min(current_rewards).item()}, total score = {score_board}")
    ...
    def evaluate(self):
        ...
        for step in tqdm(range(500)):
            ...
            with torch.no_grad():
                ...
                action, prob_logit, entropy, value = self.action_value(current_parameter, current_minimap)
            # batched environmnts
            next_parameter, next_minimap, reward, terminated, info = env.step(action.cpu().tolist()[0])
