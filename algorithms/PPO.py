...
class PPOAgent():
    def __init__(self, hyperparams):
        ...
        self.envs = EnvBatch(self.env_name, self.num_envs, self.vision, self.frames, self.skip_frames, self.is_ale)
    ...
    def action_value(self, state, action = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def train(self, num_train_episodes = 100):
        ...
        for episode in range(num_train_episodes):
            ...
            while True:
                ...
                # batched action
                with torch.no_grad():
                    action, prob_logit, entropy, value = self.action_value(current_state)
                ...
                terminated = torch.logical_or(torch.tensor(terminated), torch.tensor(truncated)).to(torch.bool).to(device)
            ...
            if self.gae:
                ...
                with torch.no_grad():
                    ...
                    for step in reversed(range(len(rewards))):
                        ...
                        current_advantage = delta+mask*self.gamma*self.gae_lambda*current_advantage
                        next_value = value
            ...
            b_states        = states.view(len(states)*self.num_envs, *tuple(states.shape)[2:])
            ...
            for optimize_epoch in range(self.num_optimize_epochs):
                ...
                for start_index in range(0, batch_size, self.minibatch_size):
                    ...
                    # compute policy loss
                    _, new_prob_logits, new_entropy, new_value=self.action_value(mb_states, mb_actions)
                    ...
                    pg_loss2=mb_advantages*torch.clip(ratio, 1-self.clip_coefficient, 1+self.clip_coefficient)
                    ...
                    loss=-pg_loss+self.value_coefficient*value_loss-self.entropy_coefficient*entropy_loss
            ...
            print(f"Training episode {episode}, reward = {np.max(max_rewards)}, steps = {total_steps}")
    ...
    def reload(self):
        print("Load model")
        try:
            state_dict=torch.load(f"../models/PPO_{self.env_name}.pth", weights_only=True)
